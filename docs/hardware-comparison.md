...
|--------|-----------|-----------|-----------|-------|
| Hamming 1024-bit throughput (M/s) | 6.8 | 1.51 | TBD | M1 4.5x faster than M2 |
| Linear scan N=~390 (µs) | 57 | 260 | TBD | Proportional to clock+cores |
| MRR content-only 256-bit | 0.353 | 0.288 | 0.174† | †M3 on public (sanitized) graph |
| MRR topology-aware 1-hop 256-bit | 0.919 | **0.919** | **0.919** | **BIT-EXACT on all three** |
| MRR topology-aware 1-hop 1024-bit | 0.919 | 0.899 | — | Bit-width parity at 256-bit |
| MRR quantum walk N=50 | 1.000 | **1.000** | **1.000** | **BIT-EXACT on all three** |
| MRR quantum walk N=200 | 0.975 | **1.000** | — | Stochastic sampling variance |
| CS best MRR (512-bit) | 0.750 | **0.815** | — | numpy 2.4 optimizations? |
| Numpy vs Python int (256-bit) | numpy slower | numpy **2.2x slower** | TBD | No VPOPCNTDQ on any machine |

†Machine 3 uses the public sanitized graph (392 nodes, labels replaced). Content-only baseline
is lower because sanitized labels reduce content similarity. TA-SDM and quantum walk MRR
are unaffected because they rely on topology (preserved during sanitization).
...
All three machines achieve perfect MRR=1.000 for quantum walks on N=50 subgraphs.
...
connected subgraphs) holds on all machines tested.
...
### Finding 5: 256-bit is the sweet spot on all machines

E3 confirms: 128-bit → 256-bit gives MRR gain; 256-bit → 512-bit gives zero MRR gain.
This result is identical on all machines despite widely different L3 cache sizes
(4 MB on M2, 12 MB on M1, 30 MB on M3). The 256-bit sweet spot is an algorithmic
property, not a cache artifact.
